...
call "%DIR_SCRIPT%message.bat" "%APP%" "checkversion %APP%"
...
call "%DIR_SCRIPT%message.bat" "%CHECK%" "!LOG_INFO_DOCKIN! [%cont_server%]"
...
call "%DIR_SCRIPT%message.bat" "%CHECK%" "!LOG_INFO_SERVER_VERSEAR! - (trytond.__version__.split('.')[:3])"
...
:tryton_version
  call "%DIR_SCRIPT%message.bat" "%CHECK%" "!LOG_INFO_VERSION!: [!CURRENT_VERSION!]"
  call "%DIR_SCRIPT%message.bat" "%CHECK%" "!LOG_INFO_VERSION!: [!CURRENT_VER_MENU!]"
...
:pg_version
  ...
  call "%DIR_SCRIPT%message.bat" "%CHECK%" "!LOG_INFO_DOCKIN! [%POSTGRES%] - [SELECT 1 FROM pg_catalog.pg_database WHERE datname='!DB_NAME!]"
  ...
  call "%DIR_SCRIPT%message.bat" "%CHECK%" "!LOG_INFO_SERVER_VERSEPO! - [SELECT version()]"
  for /f "tokens=*" %%V in ('
  ...
  if  "!CURRENT_PGALL_VERSION!" NEQ "" (
    ...
    call "%DIR_SCRIPT%message.bat" "%CHECK%" "!MESSAGE!: [!CURRENT_PGALL_VERSION!]"
  ...
  :pg_not_datname
    ...
    set "MESSAGE=!LOG_INFO_PG_VERSION:PROYECTO=%DB_NAME%!"
    call "%DIR_SCRIPT%message.bat" "%CHECK%" "!MESSAGE!: [!CURRENT_PG_VERSION!]"
    call "%DIR_SCRIPT%message.bat" "%CHECK%" "checkversion: !LOG_INFO_PROCES!"
    goto :exit
