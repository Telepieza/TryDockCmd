...
set "LOG_FILE=0"
call :logger "%CHECK%" "!LOG_INFO_DOCKCO! %TRYTON_POSTGRES%-1"
call :read_container "%TRYTON_POSTGRES%-1"
if "%LOG_FILE%"=="0" set "CURRENT_POSTGRES=%TRYTON_POSTGRES%-1"
if "%CURRENT_POSTGRES%"=="" (
   call :logger "%CHECK%" "!LOG_INFO_DOCKCO! %POSTGRES%"
...
set "LOG_FILE=0"
call :logger "%CHECK%" "!LOG_INFO_DOCKCO! %TRYTON%-%SERVER%-1"
call :read_container "%TRYTON%-%SERVER%-1"
if "%LOG_FILE%"=="0" set "CURRENT_TRYTON=%TRYTON%-%SERVER%-1"
if "%CURRENT_TRYTON%"=="" (
...
set "LOG_FILE=0"
call :logger "%CHECK%" "!LOG_INFO_DOCKCO! %TRYTON%-%CRON%-1"
call :read_container "%TRYTON%-%CRON%-1"
if "%LOG_FILE%"=="0" set "CURRENT_CRON=%TRYTON%-%CRON%-1"
if "%CURRENT_CRON%"=="" (
...
:continue
  if "%CURRENT_POSTGRES%"=="" set "CURRENT_POSTGRES=%TRYTON_POSTGRES%-1"
  if "%CURRENT_TRYTON%"=="" set "CURRENT_TRYTON=%TRYTON%-%SERVER%-1"  
  if "%CURRENT_CRON%"=="" set "CURRENT_CRON=%TRYTON%-%CRON%-1"
  if "%CURRENT_VERSION%"=="" set "CURRENT_VERSION=%TRYTON-VERSION%"
  if "%CURRENT_VER_MENU%"=="" set "CURRENT_VER_MENU=%TRYTON-VERSION%"
  if "%CURRENT_PG_VERSION%"=="" set "CURRENT_PG_VERSION=%POSTGRES_VERSION%"
  set "LOG_FILE=0"
  if "%ERR_DOCKER%" NEQ "0" (
    exit /b 2
  )
