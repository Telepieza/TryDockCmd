...
call :logger %APP% "client %cte_action%"
...
:continue
 ...
 :: Cuarto nivel puerto abierto pero no activo
  if %errorlevel% == 0 (
    ...
    " exit 0 } catch { exit 1 }"
    if %errorlevel% NEQ 0 (
      call :logger "!LOG-ERROR!" "!INSTALL_HTTP!"
      goto :error
    )
  )
  ...
  if %islocal%=="0" (
     ...
     if %errorlevel% NEQ 0 (
       call :logger "!LOG-WARN!" "!INSTALL_OFFLINE!"
       goto :error
     )
  )
...
:check_ready
  set /a "attempts+=1"
  call :logger "!LOG-INFO!" "Checking service readiness (Attempt !attempts!/!max_attempts!)..."
  ...
  if !errorlevel! equ 0 (
    call :logger "!LOG-SUCC!" "Service is fully responsive. Launching browser..."
    call "%DIR_SCRIPT%global_routines.bat" "%proyecto%" "timeout_start" "!wait_timecte!" "1"
    start "" "%url%"
    goto :exit
  )
  if !attempts! LSS !max_attempts! (
    ...
    call :logger "!LOG-ERROR!" "Service is taking too long to respond. Please refresh manually."
  )
  goto :exit
