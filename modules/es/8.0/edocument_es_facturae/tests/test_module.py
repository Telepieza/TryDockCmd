...
from trytond.modules.edocument_es_facturae.edocument import escape_x509_name
...
class EdocumentEsFacturaeTestCase(ModuleTestCase):
    ...
    def test_escape_x509_name(self):
        "Test X509 name non-ASCII escaping"
        issuer = (
            'CN=EC-Ciutadania,OU=Serveis Públics de Certificació,'
            'O=CONSORCI ADMINISTRACIO OBERTA DE CATALUNYA,C=ES')

        self.assertEqual(
            escape_x509_name(issuer),
            'CN=EC-Ciutadania,OU=Serveis P\\C3\\BAblics de '
            'Certificaci\\C3\\B3,'
            'O=CONSORCI ADMINISTRACIO OBERTA DE CATALUNYA,C=ES')
    ...
    def _test_facturae(self, version, refund_reason=None):
        ...
        with set_company(company):
            ...
            return etree.fromstring(invoice_string)
