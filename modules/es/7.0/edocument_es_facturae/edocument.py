...
def escape_x509_name(value):
    return ''.join(
        char if 32 <= ord(char) <= 126 else ''.join(
            '\\%02X' % byte for byte in char.encode('utf-8'))
        for char in value)
...
class Invoice(Model):
    ...
    def _validate_facturae(self, content, version):
        schema_file_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)),
            'xsd',
            'Facturaev%s.xml' % version.replace('.', '_'))
        with open(schema_file_path, encoding='utf-8') as schema_file:
            facturae_schema = etree.XMLSchema(file=schema_file)
            facturae_schema.assertValid(etree.fromstring(content))
    ...
    def render(self, version):
        ...
        self._validate_facturae(content, version)
        return content
    ...
    def total_gross_amount(self):
        return self.invoice.untaxed_amount

    @cached_property
    def general_discounts(self):
        return []

    @cached_property
    def general_surcharges(self):
        return []

    @cached_property
    def total_general_discounts(self):
        return sum(
            (d['amount'] for d in self.general_discounts), Decimal('0'))

    @cached_property
    def total_general_surcharges(self):
        return sum(
            (s['amount'] for s in self.general_surcharges), Decimal('0'))
...
class FacturaeSigner(XAdESSigner):
    ...
    def add_signing_certificate(self,
        ...
        for cert in signing_settings.cert_chain:
            ...
            # Use RFC4514 order and RFC2253-style UTF-8 escaping to satisfy
            # strict validators.
            etree.SubElement(issuer_serial, ds_tag("X509IssuerName"),
                nsmap=self.namespaces).text = (
                    escape_x509_name(loaded_cert.issuer.rfc4514_string()))
